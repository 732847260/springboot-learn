...
import com.springboot.lock.common.RedisCommonThing;
 ...
 */
@RestController
public class RedisThingController {

    @Autowired
    RedisCommonThing redisCommonThing;
    ...
    @RequestMapping(value = "/seckilling2")
    public void Seckilling(String targetId){
        //加锁
        long time = TIMEOUT;
        String token = UUID.randomUUID().toString();
        if(!redisCommonThing.lock(targetId,token,time)){
           // return "排队人数太多，请稍后再试.";
        ...
        //解锁
        redisCommonThing.unlock(targetId,token);
    }
